...
# Scale fixed pixel sizes (fonts, molecule radius, line widths) with the window
# so battle mode reads well on a laptop and on the large exhibit screen alike
# (design basis ~950 px tall). main.py fixes the window size before this import.
_S = max(Window.height, 700) / 950.0


def _sf(v):
    """Scale a design-pixel value to the current window height."""
    return v * _S
...
MAX_SPEED       = 550.0
REMOVE_COOLDOWN = 0.18   # min seconds between Makey/keyboard removals per side (debounces bouncy floor pads)

# Battle sabotage input: a floor pad removes one molecule from the OPPONENT's side.
# Windows cannot tell two identical boards apart, so the two boards MUST send
# different keys. Fixed, predictable mapping (no device ID needed):
#   LEFT player's pad  -> SPACE / ← / A          -> removes from the RIGHT side
#   RIGHT player's pad -> → arrow / Enter / L     -> removes from the LEFT  side
# So: wire the LEFT pad to SPACE and the RIGHT pad to the → arrow.
LEFT_KEYS  = (32, 276, 97)         # Space, Left-arrow, A
RIGHT_KEYS = (275, 13, 271, 108)   # Right-arrow, Enter, Numpad-Enter, L
...
class BattleScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self._last_remove = {'left': 0.0, 'right': 0.0}
    ...
    def _build_target_panel(self):
        ...
        self._target_sub = Label(
            text='— FIRST TO —',
            font_name=_FONT, font_size=_sf(16),
        ...
        self._target_lbl = Label(
            ...
            font_name=_FONT, font_size=_sf(26),
            halign='center', valign='middle',
    ...
    def _build_temp_labels(self):
        ...
        self._left_temp = Label(
            text='LEFT TEMP\n0.0',
            font_name=_FONT, font_size=_sf(18),
            size_hint=(0.24, 0.09),
    ...
    def _build_progress_bars(self):
        ...
        self._left_bar = Label(
            text='',
            font_name=_FONT, font_size=_sf(11),
    ...
    def _build_win_label(self):
        self._win_lbl = Label(
            text='', font_name=_FONT, font_size=_sf(64),
            size_hint=(0.70, 0.16),
            pos_hint={'center_x': 0.5, 'center_y': 0.52},
            halign='center', valign='middle',
            color=(1, 1, 0.4, 0),
            outline_color=(0.7, 0.5, 0, 1),
            outline_width=max(1, int(_sf(4))),
        )
    ...
    def _build_back_button(self):
        btn = Button(
            text='BACK', font_name=_FONT, font_size=_sf(16),
    ...
    def on_enter(self, *args):
        ...
        self._dual_makey = DualMakeyInput(
            left_cb=lambda: self._makey_remove('right'),
            right_cb=lambda: self._makey_remove('left'),
        )
    ...
    def _build_key_setup(self):
        """Setup helper shown at the bottom of the arena. On every pad/key press it
        shows which side that key removes from, so each floor pad can be tested and
        wired correctly. Fades out a couple seconds after the last press."""
        self._key_setup_lbl = Label(
            text='Makey setup:  LEFT pad = SPACE (hits RIGHT) · RIGHT pad = → arrow (hits LEFT).  Tap a pad to test.',
            font_name=_FONT, font_size=_sf(17),
            size_hint=(0.94, 0.045),
            pos_hint={'center_x': 0.5, 'y': 0.006},
            halign='center', valign='middle',
            color=(1.0, 0.85, 0.35, 0.95),
        )
        self._key_setup_lbl.bind(size=self._key_setup_lbl.setter('text_size'))
        self._root.add_widget(self._key_setup_lbl)

    def _key_down(self, window, key, scancode, codepoint, modifiers):
        # On Linux, DualMakeyInput (evdev) routes each board by physical device,
        # so ignore OS key events to avoid double-firing.
        if getattr(self, '_dual_makey', None) and self._dual_makey._devices:
            return
        if self._state != 'playing':
            return
        # Fixed, predictable mapping - no device ID and no order-dependence. The two
        # boards MUST send different keys (Windows can't tell identical ones apart):
        #   LEFT pad  -> SPACE / ← / A  -> removes from the RIGHT side (its opponent)
        #   RIGHT pad -> → / Enter / L  -> removes from the LEFT  side (its opponent)
        if key in LEFT_KEYS:
            side = 'right'
        elif key in RIGHT_KEYS:
            side = 'left'
        else:
            self._show_pad_hit(key, None)
            return
        self._makey_remove(side)
        self._show_pad_hit(key, side)

    def _makey_remove(self, team):
        """Remove one molecule from `team`, debounced so a single stomp on a bouncy
        floor pad removes exactly one molecule instead of a burst. Each side has its
        own cooldown, so the two boards never interfere with each other."""
        if self._state != 'playing':
            return
        now = Clock.get_time()
        if now - self._last_remove[team] < REMOVE_COOLDOWN:
            return
        self._last_remove[team] = now
        self._remove_mol(team)

    def _show_pad_hit(self, key, side):
        """Flash which key was pressed and which side it removed from, so each floor
        pad can be tested directly. Press one pad, then the other: if both show the
        same side, they send the same key - move one clip to a different pad."""
        lbl = getattr(self, '_key_setup_lbl', None)
        if lbl is None:
            return
        from kivy.animation import Animation
        ...
        lbl.opacity = 1
        if side is None:
            lbl.text = f'key {key} is not mapped - use SPACE (left pad) or the → arrow (right pad)'
            lbl.color = (1.0, 0.55, 0.35, 0.95)
        else:
            lbl.text = f'key {key}  →  removed from the {side.upper()} side       (LEFT pad = SPACE, RIGHT pad = → arrow)'
            lbl.color = (0.45, 0.95, 1.0, 0.95)
        (Animation(opacity=1, duration=2.5) + Animation(opacity=0, duration=1.0)).start(lbl)
    ...
    def _draw(self, dt):
        ...
        with c:
            ...
            if self._divider_opacity > 0.01:
                ...
                Line(points=[mid, ay, mid, ay + ah], width=_sf(22))
                Color(0.45, 0.92, 1.0, 0.14 * op)
                Line(points=[mid, ay, mid, ay + ah], width=_sf(12))
                Color(0.45, 0.92, 1.0, 0.60 * op)
                Line(points=[mid, ay, mid, ay + ah], width=_sf(2.8))
                # notch marks
