...
The interface scales to the window, so it looks right on any display. By default
it opens a window sized to fit your screen. For the large touchscreen it was
built for, run it full screen:

```bash
MDKIVY_FULLSCREEN=1 python main.py      # fill the display (exhibit touchscreen)
MDKIVY_WIDTH=2560 MDKIVY_HEIGHT=1440 python main.py   # or force an exact size
```
